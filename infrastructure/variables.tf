...
variable "environment" {
  ...
  validation {
    condition     = contains(["internal-sales", "internal-sales-dev", "dev", "qa", "stg", "prd"], var.environment)
    error_message = "Environment must be one of: internal-sales, internal-sales-dev, dev, qa, stg, prd."
  }
}
...
variable "instance_type" {
  ...
  default     = "t3.xlarge"
  validation {
    condition     = contains(["t2.xlarge", "t3.large", "t3.xlarge", "t3.2xlarge", "t2.micro"], var.instance_type)
    error_message = "Instance type must be one of: t2.xlarge, t3.large, t3.xlarge, t3.2xlarge, t2.micro."
  }
}
